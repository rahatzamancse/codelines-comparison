...
add_annotations <- function(p, data) {
  ...
  # Add "Trend upward!" text using annotation_custom instead of annotate
  p <- p + annotation_custom(
    grob = textGrob(
      "Trend upward!",
      gp = gpar(fontface = "bold", fontsize = 12)
    ),
    xmin = as.Date(mean(range(pred_data$date))), xmax = as.Date(mean(range(pred_data$date))),
    ymin = mean(pred_data$price) + 50, ymax = mean(pred_data$price) + 50
  )
  ...
  if (nrow(aapl_2007) > 0 && nrow(goog_2007) > 0) {
    ...
    # Add "Price gap in 2007" text using annotation_custom
    p <- p + annotation_custom(
      grob = textGrob(
        "Price gap in 2007",
        gp = gpar(fontsize = 10)
      ),
      xmin = aapl_2007$date + days(180), xmax = aapl_2007$date + days(180),
      ymin = mean(c(aapl_2007$price, goog_2007$price)), ymax = mean(c(aapl_2007$price, goog_2007$price))
    )
  }
  ...
  rect_grob <- rectGrob(
    gp = gpar(fill = NA, col = "gray", lty = "dashed")
  )

  p <- p + annotation_custom(
    grob = rect_grob,
    xmin = min(data$date) + days(180), xmax = min(data$date) + days(730),
    ymin = max(data$price) - 200, ymax = max(data$price) - 50
  )

  # Add "such empty!" text using annotation_custom
  p <- p + annotation_custom(
    grob = textGrob(
      "such empty!",
      gp = gpar(fontsize = 10)
    ),
    xmin = min(data$date) + days(455), xmax = min(data$date) + days(455),
    ymin = max(data$price) - 230, ymax = max(data$price) - 230
  )
  ...
  # Add "First Point!" text using annotation_custom
  p <- p + annotation_custom(
    grob = textGrob(
      "First Point!",
      gp = gpar(fontsize = 10)
    ),
    xmin = first_goog$date + days(365), xmax = first_goog$date + days(365),
    ymin = first_goog$price - 100, ymax = first_goog$price - 100
  )
